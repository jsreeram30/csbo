...
export class AppsEditComponent implements OnInit {
	...
	constructor(private activatedRoute: ActivatedRoute,
		...
		private creditAppsService: CreditAppsService,
	...
	ngOnInit() {
		...
		this.activatedRoute.queryParams.subscribe(params => {
			const id = +params.id;
			if (id && id > 0) {
				this.creditAppsService.getProductById(id).subscribe(res => {
					this.product = res;
	...
	initProduct() {
		...
		if (!this.product.id) {
			this.subheaderService.setBreadcrumbs([
				{ title: 'Applications', page: '/creditApps' },
				{ title: 'Create Application', page: '/creditApps/add' }
			]);
			return;
		}
		this.subheaderService.setTitle('Edit Application');
		this.subheaderService.setBreadcrumbs([
			{ title: 'Applications', page: '/creditApps' },
			{ title: 'Edit Application', page: '/creditApps/edit', queryParams: { id: this.product.id } }
		]);
	}
	...
	refreshProduct(id = 0) {
		const _refreshUrl = 'creditApps/edit?id=' + id;
	...
	addProduct(_product: ProductModel, withBack: boolean = false) {
		this.loadingSubject.next(true);
		this.creditAppsService.createProduct(_product).subscribe(res => {
			this.loadingSubject.next(false);
			if (withBack) {
				this.goBack(res.id);
			} else {
	...
	updateProduct(_product: ProductModel, withBack: boolean = false) {
		...
		// Update Product
		// tslint:disable-next-line:prefer-const
		let tasks$ = [this.creditAppsService.updateProduct(_product)];
		...
		forkJoin(tasks$).subscribe(res => {
			...
			if (withBack) {
				...
				const message = `Appliation successfully has been saved.`;
	...
	getComponentTitle() {
		...
		result = `Edit Application - ${this.product.manufacture} ${this.product.model}, ${this.product.modelYear}`;
		return result;
	}
