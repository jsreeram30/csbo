...
export class AppsListComponent implements OnInit {
	...
	ngOnInit() {
		...
		**/
		...
		// Init DataSource
		this.dataSource = new ProductsDataSource(this.creditAppsService);
		let queryParams = new QueryParamsModel({});
		// Read from URL itemId, for restore previous state
		this.route.queryParams.subscribe(params => {
			if (params.id) {
				queryParams = this.creditAppsService.lastFilter$.getValue();
	...
	restoreState(queryParams: QueryParamsModel, id: number) {
		if (id > 0) {
			this.creditAppsService.getProductById(id).subscribe((res: ProductModel) => {
	...
	deleteProduct(_item: ProductModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.creditAppsService.deleteProduct(_item.id).subscribe(() => {
	...
	deleteProducts() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.creditAppsService.deleteProducts(idsForDeletion).subscribe(() => {
				this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
				this.loadProductsList();
	...
	updateStatusForProducts() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.creditAppsService.updateStatusForProduct(this.selection.selected, +res).subscribe(() => {
